...
namespace DoctorERP
{
    public partial class FrmWorkFeeRpt : KryptonForm
    {
        DgvFilterManager dgvManager;
        string ReportTitle = string.Empty;


        public FrmWorkFeeRpt(string ReportTitle)
        {
            InitializeComponent();
        ...
        void FillGrid()
        {
            ...
            if (CmbChangeDate.SelectedIndex == 0)
                ...
                vwWorkFee.Fill(dtStartDate.Value, dtEndDate.Value);

            DataGridMain.DataSource = vwWorkFee.dtData;
        ...
        void InitGrid()
        {
            vwWorkFee.Fill("Guid", Guid.Empty);
            DataGridMain.DataSource = vwWorkFee.dtData;

            DataGUIAttribute.FillGrid(DataGridMain, typeof(vwWorkFee));
        ...
        private void DataGridMain_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                FrmBillHeader frm = new FrmBillHeader(Guid.Empty, false, bill.billtype, new List<tbLand>());
        ...
        private void CalcTotal()
        {
            ...
            decimal workfeevalue = CalcColumnTotal(DataGridMain, DataGridMain.Columns["workfeevalue"]);
            ...
            LblAmountOut.Text = string.Format("إجمالي الضريبة {0:" + DataGUIAttribute.CurrencyFormat + "}", workfeevalue);
        ...
        private bool Readyreport(FastReport.Report rpt)
        {
            ...
            Reports.InitReport(rpt, "workfeenetrpt.frx", false);
