...
namespace DoctorERP
{
    public partial class FrmBuildingFeeNumberRpt : KryptonForm
    {
        DgvFilterManager dgvManager;
        string ReportTitle = string.Empty;


        public FrmBuildingFeeNumberRpt(string ReportTitle)
        {
            InitializeComponent();
        ...
        void FillGrid()
        {
            ...
            if (CmbChangeDate.SelectedIndex == 0)
                vwBuildingFeeNumbers.Fill();
            else if (CmbChangeDate.SelectedIndex != 0)
                vwBuildingFeeNumbers.Fill(dtStartDate.Value, dtEndDate.Value);

            DataGridMain.DataSource = vwBuildingFeeNumbers.dtData;
        ...
        void InitGrid()
        {
            vwBuildingFeeNumbers.Fill("Guid", Guid.Empty);
            DataGridMain.DataSource = vwBuildingFeeNumbers.dtData;

            DataGUIAttribute.FillGrid(DataGridMain, typeof(vwBuildingFeeNumbers));
        ...
        private void CalcTotal()
        {
            ...
            decimal buildingfeevalue = CalcColumnTotal(DataGridMain, DataGridMain.Columns["buildingfeevalue"]);
            //decimal remain = CalcColumnTotal(DataGridMain, DataGridMain.Columns["remain"]);

            LblAmountIn.Text = string.Format("القيمة {0:" + DataGUIAttribute.CurrencyFormat + "}", price);
            LblAmountOut.Text = string.Format("إجمالي الضريبة {0:" + DataGUIAttribute.CurrencyFormat + "}", buildingfeevalue);
        ...
        private bool Readyreport(FastReport.Report rpt)
        {
            ...
            Reports.InitReport(rpt, "buildingfeenumberrpt.frx", false);
