...
namespace DoctorERP
{
    public partial class FrmContractRpt : KryptonForm
    {
        DgvFilterManager dgvManager;
        string ReportTitle = string.Empty;


        public FrmContractRpt(string ReportTitle)
        {
            InitializeComponent();
        ...
        void FillGrid()
        {
            ...
            if (CmbChangeDate.SelectedIndex == 0)
                if (CmbPayType.SelectedIndex == 0)
                    vwContractRpt.Fill("Status", "مباع");
                else
                    vwContractRpt.Fill("Status", "مرتجع");

            else if (CmbChangeDate.SelectedIndex != 0)
                if (CmbPayType.SelectedIndex == 0)
                    vwContractRpt.Fill(dtStartDate.Value, dtEndDate.Value , "مباع");
                else
                    vwContractRpt.Fill(dtStartDate.Value, dtEndDate.Value, "مرتجع");


            DataGridMain.DataSource = vwContractRpt.dtData;
        ...
        void InitGrid()
        {
            vwContractRpt.Fill("Guid", Guid.Empty);
            DataGridMain.DataSource = vwContractRpt.dtData;

            DataGUIAttribute.FillGrid(DataGridMain, typeof(vwContractRpt));
        ...
        private void DataGridMain_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                FrmBillHeader frm = new FrmBillHeader(bill.guid, false, bill.billtype, new List<tbLand>());
        ...
        private bool Readyreport(FastReport.Report rpt)
        {
            ...
            Reports.InitReport(rpt, "contractrpt.frx", false);
