...
{
    public partial class UCBills : System.Windows.Forms.UserControl
    {
        ...
        List<tbLand> SellLands;
        tbAgent Client = new tbAgent();

        public UCBills(Guid _Guid, List<tbLand> _SellLands, tbAgent _Client, int BillType)
        {
            ...
            MainContainer.BackgroundImage = Real_Estate_Management.Properties.Resources.Background;
            MainContainer.BackgroundImageLayout = ImageLayout.Stretch;

            CommandBarButton[] radBarButton = new[] { BtnResfresh, BtnDelete, BtnNew, BtnEdit, BtnExit };
            ...
            CommandBarDropDownButton[] radDropDownButton = new[] { BtnImport, BtnExport, BtnPrint };
            ...
            RadMenuItem[] radMenuItem = new[] { BtnImportExcel, BtnExportExcelData, BtnEcelExport, BtnEmailExport, BtnPdfExport, BtnWordExport, BtnSentToPrinter, BtnDesign, BtnPreview };
            ...
            CurrentGuid = _Guid;
            SellLands = _SellLands;
            Client = _Client;

            if (BillType == 0)
            {
                Text = "عقد بيع";
                radHeaderLabel.Text = "عقد بيع أرض";
            }
            else if (BillType == 1)
            {
                Text = "مرتجع بيع";
                radHeaderLabel.Text = "عقد مرتجع بيع أرض";
            }
            else if (BillType == 2)
            {
                ...
                radHeaderLabel.Text = "عقد بيع خارجي (صوري)";
            }
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            {
                if (IsDirty && !(radDGVBills.Focus())) { TackAction(); }
            }
        ...
        private void SetData()
        {
            ...
            foreach (RadPanel panel in panels)
            {
                IEnumerable<RadControl> controls = panel.Controls.OfType<RadControl>().Where(control => !control.Name.StartsWith("rad"));
            ...
            {
                var ObjLawer = tbBillheader.lstData.Where(u => u.guid == CurrentGuid).ToList();
                if (ObjLawer.Any())
                {
                    //FillDataGridParenters(CurrentGuid);
                    int index = tbBillheader.lstData.IndexOf(ObjLawer.First());
                    Bs.Position = index;
                    return;
                }
            }
        ...
        private void radButton1_Click(object sender, EventArgs e)
        {
            ...
            //        for (int i = startidx; i < radDGVBills.RowCount; i++)
            //        {
            //            Guid guid = new Guid(radDGVBills.GetRowCellValue(i, ColLandGuid).ToString());
            ...
            //        for (int i = 0; i < radDGVBills.RowCount; i++)
            //        {
            //            Guid guid = new Guid(radDGVBills.GetRowCellValue(i, ColLandGuid).ToString());
        ...
        ////public void FillDataGridBills(Guid parentguid)
        ////{

        ////    //TbParenters_Rep.Fill(parentguid);
        ////    //DGVBills.DataSource = TbParenters_Rep.lstData;

        ////    //DGVBills.Columns[0].IsVisible = false;
        ////    //DGVBills.Columns[1].IsVisible = false;
        ////    //DGVBills.Columns[2].IsVisible = false;
        ////    //DGVBills.Columns[3].IsVisible = false;

        ////    //DGVBills.Columns[4].HeaderText = "اسم الشريك";
        ////    //DGVBills.Columns[5].HeaderText = "الرقم القومي";
        ////    //DGVBills.Columns[6].HeaderText = "الموبايل";
        ////    //DGVBills.Columns[7].HeaderText = "ملاحظات";

        ////    //DGVBills.AutoSizeColumnsMode = GridViewAutoSizeColumnsMode.Fill;


        ////    //foreach (GridViewDataColumn col in this.DGVBills.Columns)
        ////    //{
        ...
        void CalcBillBody()
        {
            ...
            ////for (int i = 0; i < radDGVBills.RowCount; i++)
            ////{
            ////    Guid landguid = new Guid(radDGVBills.Rows[i].Cells[0].ToString());


            ////    decimal bodytotal;
            ////    decimal.TryParse(radDGVBills.Rows[i].Cells[8].ToString(), out bodytotal);

            ////    decimal bodynettotal;
            ////    decimal.TryParse(radDGVBills.Rows[i].Cells[14].ToString(), out bodynettotal);


            ////    decimal bodydiscounttotalvalue;
            ////    decimal.TryParse(radDGVBills.Rows[i].Cells[9].ToString(), out bodydiscounttotalvalue);

            ////    decimal bodydiscountfeevalue;
            ////    decimal.TryParse(radDGVBills.Rows[i].Cells[13].ToString(), out bodydiscountfeevalue);

            ////    decimal bodyvatfeevalue;
            ////    decimal.TryParse(radDGVBills.Rows[i].Cells[12].ToString(), out bodyvatfeevalue);

            ////    decimal bodybuildingfeevalue;
            ////    decimal.TryParse(radDGVBills.Rows[i].Cells[10].ToString(), out bodybuildingfeevalue);

            ////    decimal bodyworkfeevalue;
            ////    decimal.TryParse(radDGVBills.Rows[i].Cells[11].ToString(), out bodyworkfeevalue);

            ////    decimal bodynetworkfeevalue;
            ////    decimal.TryParse(radDGVBills.Rows[i].Cells[0].ToString(), out bodynetworkfeevalue);

            ////    decimal bodypricetotal;
            ////    decimal.TryParse(radDGVBills.Rows[i].Cells[0].ToString(), out bodypricetotal);


            ////    string status = radDGVBills.Rows[i].Cells[0].ToString();
