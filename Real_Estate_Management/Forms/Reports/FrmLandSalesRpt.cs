...
{
    public partial class FrmLandSalesRpt : KryptonForm
    {
        DgvFilterManager dgvManager;
        string ReportTitle = string.Empty;


        public FrmLandSalesRpt(string ReportTitle)
        {
            InitializeComponent();
        ...
        void FillGrid()
        {
            ...
            if (CmbChangeDate.SelectedIndex == 0)
                ...
                vwLandSales.Fill(dtStartDate.Value, dtEndDate.Value);

            DataGridMain.DataSource = vwLandSales.dtData;
        ...
        void InitGrid()
        {
            vwLandSales.Fill("Guid", Guid.Empty);
            DataGridMain.DataSource = vwLandSales.dtData;

            DataGUIAttribute.FillGrid(DataGridMain, typeof(vwLandSales));
        ...
        private void DataGridMain_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                FrmBillHeader frm = new FrmBillHeader(Guid.Empty, false, 0, new List<tbLand>());
        ...
        private bool Readyreport(FastReport.Report rpt)
        {
           // Reports.GenerateReport(ReportTitle, DataGridMain, dgvManager.mBoundDataView.ToTable());
            ...
            Reports.InitReport(rpt, "landsalesrpt.frx", false);
