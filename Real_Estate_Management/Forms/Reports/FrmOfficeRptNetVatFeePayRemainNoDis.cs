...
{
    public partial class FrmOfficeRptNetVatFeePayRemainNoDis : KryptonForm
    {
        DgvFilterManager dgvManager;
        string ReportTitle = string.Empty;


        public FrmOfficeRptNetVatFeePayRemainNoDis(string ReportTitle)
        {
            InitializeComponent();
        ...
        void FillGrid()
        {
            ...
            if (CmbChangeDate.SelectedIndex == 0)
                vwOfficeRptFull.Fill();
            else if (CmbChangeDate.SelectedIndex != 0)
                vwOfficeRptFull.Fill(dtStartDate.Value, dtEndDate.Value);

            DataGridMain.DataSource = vwOfficeRptFull.dtData;
        ...
        void InitGrid()
        {
            vwOfficeRptFull.Fill("Guid", Guid.Empty);
            DataGridMain.DataSource = vwOfficeRptFull.dtData;

            DataGUIAttribute.FillGrid(DataGridMain, typeof(vwOfficeRptFull));
        ...
        private void DataGridMain_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                FrmBillHeader frm = new FrmBillHeader(guid, false, 0, new List<tbLand>());
        ...
        private void CalcTotal()
        {
            ...
            decimal netprice = CalcColumnTotal(DataGridMain, DataGridMain.Columns["netprice"]);
            //decimal remain = CalcColumnTotal(DataGridMain, DataGridMain.Columns["remain"]);

            decimal amountin = CalcColumnTotal(DataGridMain, DataGridMain.Columns["feepayments"]);
            decimal remain = CalcColumnTotal(DataGridMain, DataGridMain.Columns["feeremain"]);

            LblTotalPrice.Text = string.Format("القيمة الدفترية {0:" + DataGUIAttribute.CurrencyFormat + "}", price);
            LblTotalNetPrice.Text = string.Format("صافي القيمة {0:" + DataGUIAttribute.CurrencyFormat + "}", netprice);
        ...
        private bool Readyreport(FastReport.Report rpt)
        {
            ...
            Reports.InitReport(rpt, "OfficeRptNetVatFeePayRemaindis.frx", false);
